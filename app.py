...
# =========================================================
# 1. إعداد الصفحة وتنسيق الألوان الحيوي
# =========================================================
...
# إضافة تنسيقات CSS لجعل الأزرار ممتعة وحيوية
st.markdown("""
    <style>
    .stButton>button {
        border-radius: 20px;
...
# =========================================================
# 2. قاعدة بيانات التدريب (كاملة كما طلبتها)
# =========================================================
...
# =========================================================
# 5. إدارة الحالة (Session State) للمسح
# =========================================================
...
st.markdown("### Message Safety Detection System (Bilingual)")
...
# تحديث القيمة في الـ Session State
st.session_state.user_input = user_message


# =========================================================
# 8. الأزرار (فحص، مسح، خروج)
# =========================================================

col1, col2, col3 = st.columns(3)

with col1:
    check_btn = st.button("🔍 فحص الرسالة")

with col2:
    clear_btn = st.button("🗑️ مسح الكل")

with col3:
    exit_btn = st.button("🚪 خروج")
...
if exit_btn:
    st.success("👋 شكراً لاستخدامك النظام. يمكنك إغلاق الصفحة بأمان.")
    st.stop()
...
if check_btn:
    ...
    if not user_message.strip():
        st.warning("⚠️ الرجاء كتابة رسالة أولاً ليتمكن النظام من فحصها!")
    else:
        ...
        if prediction == "Suspicious Message":
            st.error("🚨 تحذير: الرسالة تبدو مشبوهة (Suspicious Message)")
            st.markdown("""
                **السبب (Reason):**
                تحتوي الرسالة على أنماط أو كلمات تشبه الرسائل الاحتيالية أو الإعلانية المزعجة.
            ...
            st.success("✅ الرسالة تبدو آمنة (Safe Message)")
            st.markdown("""
                **السبب (Reason):**
                الرسالة تشبه نمط المحادثات الطبيعية الموجودة في بيانات التدريب.

                **التوصية (Advice):**
                يمكنك التعاملها بشكل طبيعي، مع الاستمرار في الحذر من الروابط والطلبات غير المتوقعة.
            """)

        st.info(f"🤖 درجة ثقة النموذج التقريبية: {max_probability * 100:.2f}%")


# =========================================================
# 12. معلومات عن النظام
# =========================================================
